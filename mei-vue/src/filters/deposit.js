...
export function formatMoneyNumber (moneyNum, type) {
  if (type === 'blur') {
    let result = isNaN((1 * moneyNum).toFixed(2)) ? (Number(0).toFixed(2)) : (1 * moneyNum).toFixed(2)
    return /\./.test(result) ? result.replace(/(\d{1,3})(?=(\d{3})+\.)/g, '$1,') : result.replace(/(\d{1,3})(?=(\d{3})+\b)/g, '$1,')
  } else {
    moneyNum = isNaN((1 * moneyNum).toFixed(4)) ? (Number(1).toFixed(4)) : (1 * moneyNum).toFixed(4)
